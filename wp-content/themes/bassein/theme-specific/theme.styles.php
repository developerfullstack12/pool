...
if (!function_exists('bassein_customizer_get_css')) {

	function bassein_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = bassein_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('bassein_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(bassein_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = bassein_get_scheme_colors($scheme);
...
table > tbody > tr:nth-child(1) > td{
	background-color: {$colors['extra_bg_color']};
	color: {$colors['inverse_link']};
	border-color: {$colors['extra_bd_color_09']};
}
...
form.mc4wp-form .mc4wp-form-fields input[type="email"]::-webkit-input-placeholder{ color: {$colors['input_text']}; opacity: 1;}
form.mc4wp-form .mc4wp-form-fields input[type="email"]::-moz-placeholder{ color: {$colors['input_text']};opacity: 1; }
form.mc4wp-form .mc4wp-form-fields input[type="email"]:-ms-input-placeholder{ color: {$colors['input_text']};opacity: 1; }
...
.theme_button.color_style_link2, .comments_wrap .form-submit input[type="submit"] {
...
.sticky .post_categories a:hover{
	color: {$colors['text_dark']};
}
...
.sticky .post_meta_item:before,
.sticky .post_meta_item:after{
	color: {$colors['inverse_link']};
}
.sticky .post_meta .post_meta_item.post_author, .sticky .post_meta .post_meta_item.post_author:after{
	color: {$colors['inverse_link']};
}
.sticky .post_meta .post_meta_item.post_author:hover, .sticky .post_meta .post_meta_item.post_author:hover:after{
	color: {$colors['text_dark']};
}
...
.without_bg_image.top_panel_default .top_panel_title{
	background-color: {$colors['alter_bg_color']};
}
...
.bassein_tabs .bassein_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.bassein_tabs .bassein_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.post_meta .post_meta_item.post_author, .post_meta .post_meta_item.post_author:after{
	color: {$colors['text_dark']};
}
.post_meta .post_meta_item.post_author:hover, .post_meta .post_meta_item.post_author:hover:after{
	color: {$colors['text_link']};
}
...
CSS;
					...
					$rez = apply_filters('bassein_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'bassein_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>