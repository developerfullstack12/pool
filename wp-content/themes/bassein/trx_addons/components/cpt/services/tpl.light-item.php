...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
												...
												'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('avatar'), 'services-light')
																														),
															'services-light'
															)
										);
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			...
			?></a></<?php echo esc_html($title_tag); ?>><?php

		// desc
		if (!empty($meta['description'])) {
		?><div class="sc_services_item_description"><?php echo esc_html($meta['description']); ?></div><?php
		}
		// Price
		if (!empty($meta['price'])) {
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			<div class="sc_services_item_content"><?php bassein_the_excerpt_max_charlength(70); ?></div>
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button color_style_link3 sc_button_default sc_button_size_small', 'sc_services', $args)); ?>"><?php esc_html_e('more info', 'bassein'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
